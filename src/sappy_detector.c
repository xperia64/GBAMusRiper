...
// Selectsong data used in Mother 3
static uint8_t m3_bin_selectsong[0x1E] =
{
...
static long m4a_searchblock(uint8_t *gbarom, size_t gbasize)
{
	...
	while (m4a_selectsong_search_offset != -1)
	{
		...
		// If the default one fails, try the m3 bin		
		if (m4a_selectsong_offset == -1)
		{
			m4a_selectsong_offset = memsearch(gbarom, gbasize, m3_bin_selectsong, sizeof(m3_bin_selectsong), m4a_selectsong_search_offset, 1, 0);
		}		
		if (m4a_selectsong_offset != -1)
		{
#ifdef _DEBUG
